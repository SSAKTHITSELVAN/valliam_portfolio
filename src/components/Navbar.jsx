...
//   const navData = [
//     { name: 'Home', href: '/' }, // Change to root path
//     {
//       name: 'Technology Solution',
//       children: [
//         { name: 'Edge AI', path: '/edge-ai' }, // Object with path
//         'Low Power AI', 'IoT', 'Connectivity', 'Smart HMI',
//         'Cybersecurity', 'FuSa', 'Digital Twins', 'Next-Gen Mobility'
//       ]
//     },
//     { name: 'Company', href: '/company' },
...
//         <Link to="/">
//           <img src={logoImage} alt="Valliam" className="h-10" />
//         </Link>
...
//               {item.children ? (
//                 <>
//                   <button className="flex items-center gap-1 font-bold text-gray-800 hover:text-[#007BFF]" style={{ fontFamily: 'Rajdhani' }}>
//                     {item.name} <ChevronDown size={16} />
//                   </button>
//                   <div className="absolute top-full left-0 hidden group-hover:block w-64 bg-white shadow-xl border border-gray-100 p-4">
//                     {item.children.map((child) => (
//                       typeof child === 'object' ? (
//                         <Link 
//                           key={child.name} 
//                           to={child.path} 
//                           className="block py-2 text-gray-600 hover:text-[#007BFF]"
//                         >
//                           {child.name}
//                         </Link>
//                       ) : (
//                         <span key={child} className="block py-2 text-gray-400 cursor-not-allowed">{child}</span>
//                       )
//                     ))}
//                   </div>
//                 </>
...
//                 <Link 
//                   to={item.href} 
//                   className="font-bold text-gray-800 hover:text-[#007BFF]" 
    ...
    <nav className="navbar sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm transition-all duration-300">
      ...
      <div className="max-w-[1440px] mx-auto px-6 py-4">
        <div className="flex justify-between items-center h-20">
          ...
          {/* Logo Section */}
          <Link to="/" className="flex items-center gap-4 cursor-pointer group flex-shrink-0">
          ...
          <div className="hidden xl:flex items-center gap-6">
            {navData.map((item) => (
              ...
              >
                {item.children ? (
                  <button className="flex items-center gap-1 px-3 py-2 text-[20px] font-bold transition-all duration-300 rounded hover:bg-gray-50" 
                    style={{ color: activeDropdown === item.name ? '#007BFF' : '#000000', fontFamily: 'IBM Plex Sans, sans-serif' }}>
                    {item.name}
                    <ChevronDown size={20} className={`transition-transform duration-300 ${activeDropdown === item.name ? 'rotate-180' : ''}`} />
                  </button>
                ) : (
                  <Link 
                    to={item.href} 
                    className="px-3 py-2 text-[20px] font-bold transition-all duration-300 rounded relative group"
                    style={{ color: '#000000', fontFamily: 'IBM Plex Sans, sans-serif' }}
                  >
                    {item.name}
                    <span className="absolute bottom-4 left-3 right-3 h-0.5 bg-[#007BFF] scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></span>
                  </Link>
                ...
                {item.children && (
                  ...
                  }`} style={{ borderTop: '3px solid #007BFF', borderRadius: '0 0 4px 4px' }}>
                    <div className="py-2">
                      {item.children.map((subItem) => (
                        <Link
                          key={subItem.name}
                          to={subItem.path}
                          className="block px-6 py-3 text-base font-medium transition-all duration-200 border-l-4 border-transparent hover:bg-gray-50 hover:border-[#007BFF] hover:text-[#007BFF]"
                          style={{ color: '#333333', fontFamily: 'IBM Plex Sans, sans-serif' }}
                        >
                          {subItem.name}
                        </Link>
                      ))}
            ...
            <Link to="/contact" className="flex items-center gap-2 px-6 py-3 ml-4 text-[18px] font-bold text-white transition-all duration-300 hover:-translate-y-0.5 shadow-md hover:shadow-lg rounded relative overflow-hidden group" style={{ backgroundColor: '#E60000', fontFamily: 'Rajdhani, sans-serif' }}>
              <span className="absolute inset-0 w-full h-full bg-gradient-to-r from-red-600 to-red-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></span>
