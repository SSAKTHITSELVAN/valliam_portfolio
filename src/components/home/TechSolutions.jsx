...
import { 
  ...
  ArrowRight, CheckCircle2, PauseCircle, PlayCircle 
} from 'lucide-react';
    ...
    <section className="py-24 bg-[#F8F9FA] relative overflow-hidden" id="technology">
      ...
      <div className="max-w-[1440px] mx-auto px-6 relative z-10">
        ...
        <div className="mb-16">
          ...
          <div className="hidden lg:flex items-center gap-2 mt-4 text-gray-400 text-sm font-medium">
             {isAutoPlaying ? (
               <><div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div> Auto-Cycling Enabled</>
             ) : (
               <><div className="w-2 h-2 rounded-full bg-gray-400"></div> Manual Mode Active</>
             )}
          </div>
        </div>

        {/* =========================================================
            DESKTOP VIEW: Split Dashboard (Hidden on Mobile)
        ========================================================= */}
        <div className="hidden lg:flex flex-row gap-16">
          
          {/* LEFT: Dynamic Monitor */}
          <div className="w-5/12 relative">
             <div className="sticky top-32">
                <div className="bg-white p-12 rounded-sm shadow-2xl border-t-8 transition-all duration-500 ease-in-out relative overflow-hidden group"
                   ...
                   {/* Icon */}
                   <div className="relative z-10 w-24 h-24 flex items-center justify-center rounded-full bg-gray-50 mb-8 transition-colors duration-500"
                        style={{ color: solutions[activeId].color }}>
                     {solutions[activeId].icon}
                   </div>

                   {/* Title & Desc - Increased Font Size */}
                   <h3 className="relative z-10 text-4xl font-bold mb-6 transition-all duration-300 leading-tight" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                     {solutions[activeId].title}
                   </h3>
                   <p className="relative z-10 text-xl text-gray-600 leading-relaxed mb-10 min-h-[120px]" style={{ fontFamily: 'IBM Plex Sans, sans-serif' }}>
                     {solutions[activeId].desc}
                   </p>

                   {/* Features */}
                   <div className="relative z-10 space-y-4">
                      <p className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">Key Capabilities</p>
                      {solutions[activeId].features.map((feature, idx) => (
                        <div key={idx} className="flex items-center gap-3 text-gray-700 text-lg">
                           <CheckCircle2 size={22} style={{ color: solutions[activeId].color }} />
                   ...
                   <div className="relative z-10 mt-12 pt-8 border-t border-gray-100">
                      <button className="flex items-center gap-2 font-bold transition-all hover:gap-4 uppercase tracking-widest text-base"
            ...
            <div className="grid grid-cols-1 gap-3">
              {solutions.map((item, index) => (
                ...
                >
                  ...
                  {/* Progress Bar for Auto-Play (Only shows on active item if playing) */}
        ...
        {/* =========================================================
            MOBILE VIEW: Vertical Stream (Low Resistance Flow)
            Shown ONLY on screens smaller than lg (tablets/phones)
        ========================================================= */}
           ...
           {solutions.map((item) => (
             ...
             >
                <div className="flex items-start gap-4 mb-6">
                   <div className="p-3 bg-gray-50 rounded-full text-gray-700" style={{ color: item.color }}>
                      {item.icon}
                   </div>
                   <h3 className="text-3xl font-bold leading-none mt-2" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                     {item.title}
                   </h3>
                </div>
                
                <p className="text-lg text-gray-600 mb-6 leading-relaxed" style={{ fontFamily: 'IBM Plex Sans, sans-serif' }}>
                  {item.desc}
                </p>

                <div className="space-y-3 bg-gray-50 p-5 rounded-sm">
